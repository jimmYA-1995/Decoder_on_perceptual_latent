...
class Generator(nn.Module):
    def __init__(
        ...
        # TODO: replace mappint network with VAE encoder
    ...
    def forward(self, latents_in, labels_in=None, skip_mapping=False, return_latents=False):
        if skip_mapping:
            # not style mixing for now
            dlatents = latents_in[0].unsqueeze(1).repeat(1, self.num_layers, 1) 
        elif len(latents_in) == 2:
    ...
    # def get_latent(self, latents_in, labels_in=None):
    #     assert not isinstance(latents_in, list), "don't use style mixing when training encoder variational bayes"
    #     dlatents = self.mapping_network(latents_in, labels_in, dlatent_broadcast=None)
    #     return dlatents
